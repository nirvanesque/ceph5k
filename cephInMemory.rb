...
opts = Trollop::options do
  version "ceph-deploy 0.0.2 (c) 2015-16 Anirvan BASU, INRIA RBA"
  banner <<-EOS
...
argInMemory = opts[:inMemory] # In-memory OSD only using shared memory.
argMaxMem = opts[:maxMem] # Maximum % memory allowed from RAM for in-memory OSD.
...
puts "Option for In-memory OSDs: #{argInMemory}\n"
puts "Maximum % memory allowed from RAM: #{argMaxMem}\n" + "\n"
...
if argInMemory # Option for running OSDs on /dev/shm
   ...
   osdNodes.each do |node|    # loop over all OSD nodes
     ...
     mainMemory = parsedResult["main_memory"] # Get size of RAM
     ramSize = parsedResult["ram_size"].to_i
     allowedRAM = (argMaxMem * ramSize / 1000000000).to_i

     Cute::TakTuk.start([node], :user => "root") do |tak|
         tak.exec!("umount /tmpfs")
         tak.exec!("mkdir -p /osd.#{osdIndex}")
         tak.exec!("mount -t tmpfs -o size=#{allowedRAM}G,nr_inodes=5k,mode=755 tmpfs /osd.#{osdIndex}")
         tak.loop()
     end
     Cute::TakTuk.start([monitor], :user => "root") do |tak|
     ...
     puts "Prepared & activated OSD.#{osdIndex} on: #{nodeShort}:/dev/shm.\n"
     osdIndex += 1
   end # loop over all OSD nodes

else # Option for normal OSD, one per node
...
end # if-else for running OSDs on /dev/shm
